"""Experiment 2: noise robustness across sigma in {0.01, 0.05, 0.10}.

Loads the model trained in experiment 1 and the tuned classical params, then
evaluates all three methods on a fresh test set per noise level. Produces
plot 6 and appends rows to results.csv.
"""
...
def main() -> None:
    ...
    for sigma in SIGMAS:
        ...
        test_set = RPCADataset(
            N_TEST, N, RANK, SPARSITY, sigma, seed=SEED + 100 + int(sigma * 1000)
        )
    ...
    plot_robustness_two_panel(
        ...
        save_path=outputs / "plot_6_noise_robustness.png",
    )
    ...
    if csv_path.exists():
        ...
        df_old = df_old[~df_old["experiment"].astype(str).str.startswith("exp2")]
